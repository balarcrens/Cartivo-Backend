...
exports.getAddresses = async (req, res) => {
    try {
        const addresses = await pool.query('SELECT * FROM public.user_addresses WHERE user_id = $1 ORDER BY is_default DESC, created_at DESC', [req.user.id]);
...
exports.addAddress = async (req, res) => {
    try {
        ...
        if (is_default) {
            await pool.query('UPDATE public.user_addresses SET is_default = false WHERE user_id = $1', [req.user.id]);
        }

        const newAddress = await pool.query(
            `INSERT INTO public.user_addresses (user_id, full_name, phone, address_line1, address_line2, city, state, country, pincode, is_default) 
...
exports.deleteAddress = async (req, res) => {
    try {
        const result = await pool.query('DELETE FROM public.user_addresses WHERE id = $1 AND user_id = $2', [req.params.id, req.user.id]);
...
exports.setDefaultAddress = async (req, res) => {
    try {
        await pool.query('UPDATE public.user_addresses SET is_default = false WHERE user_id = $1', [req.user.id]);
        const updatedAddress = await pool.query(
            'UPDATE public.user_addresses SET is_default = true WHERE id = $1 AND user_id = $2 RETURNING *',
