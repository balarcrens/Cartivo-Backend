...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            let dbPrice = 0;
            if (item.variant_id) {
                const variantRes = await client.query('SELECT price, stock FROM public.product_variants WHERE id = $1', [item.variant_id]);
                ...
                const productRes = await client.query('SELECT price, stock FROM public.products WHERE id = $1', [item.product_id]);
        ...
        if (coupon_id) {
            const couponRes = await client.query('SELECT * FROM public.coupons WHERE id = $1 AND status = $2', [coupon_id, 'active']);
        ...
        if (payment_method === 'Cash on Delivery') {
            ...
            const orderRes = await client.query(
                `INSERT INTO public.orders (user_id, total_price, shipping_address, coupon_id, discount_amount, final_price, payment_method, status, payment_status) 
            ...
            for (const item of processedItems) {
                await client.query(
                    `INSERT INTO public.order_items (order_id, product_id, variant_id, quantity, price, total_price) 
                     VALUES ($1, $2, $3, $4, $5, $6)`,
            ...
            for (const item of processedItems) {
                if (item.variant_id) {
                    await client.query('UPDATE public.product_variants SET stock = stock - $1 WHERE id = $2', [item.quantity, item.variant_id]);
                } else {
                    const stockRes = await client.query('UPDATE public.products SET stock = stock - $1 WHERE id = $2 RETURNING stock', [item.quantity, item.product_id]);
                    if (stockRes.rows[0].stock === 0) {
                        await client.query("UPDATE public.products SET status = 'out_of_stock' WHERE id = $1", [item.product_id]);
                    }
                }
            }
            ...
            if (applied_coupon_id) {
                await client.query('UPDATE public.coupons SET usage_count = usage_count + 1 WHERE id = $1', [applied_coupon_id]);
            }
            // Clear cart
            await client.query('DELETE FROM public.cart_items WHERE user_id = $1', [req.user.id]);
            // Status History
            await client.query('INSERT INTO public.order_status_history (order_id, status, comment) VALUES ($1, $2, $3)', [order.id, 'pending', 'Order placed (COD)']);
...
exports.verifyPayment = async (req, res) => {
    ...
    try {
        ...
        const orderRes = await client.query(
            `INSERT INTO public.orders (user_id, total_price, shipping_address, coupon_id, discount_amount, final_price, payment_method, status, payment_status, razorpay_order_id, razorpay_payment_id, razorpay_signature) 
        ...
        for (const item of processedItems) {
            if (item.variant_id) {
                const stockRes = await client.query(
                    'UPDATE public.product_variants SET stock = stock - $1 WHERE id = $2 AND stock >= $1 RETURNING stock',
                    [item.quantity, item.variant_id]
                );
                if (stockRes.rowCount === 0) throw new Error('Insufficient stock for one or more items');
            } else {
                const stockRes = await client.query(
                    'UPDATE public.products SET stock = stock - $1 WHERE id = $2 AND stock >= $1 RETURNING stock',
        ...
        // Status History
        await client.query(
            'INSERT INTO public.order_status_history (order_id, status, comment) VALUES ($1, $2, $3)',
            [order.id, 'processing', 'Payment verified and order created successfully via Razorpay']
        );

        // Record Payment
        await client.query(
            `INSERT INTO public.payments (order_id, method, transaction_id, status, amount, payment_details) 
             VALUES ($1, $2, $3, $4, $5, $6)`,
...
exports.getOrder = async (req, res) => {
    try {
        const order = await pool.query('SELECT * FROM public.orders WHERE id = $1', [req.params.id]);
        ...
        const items = await pool.query(
            `SELECT oi.*, p.name as product_name, p.images->>0 as product_image, p.slug,
             ...
             FROM public.order_items oi 
             JOIN public.products p ON oi.product_id = p.id
             LEFT JOIN public.product_variants v ON oi.variant_id = v.id
             WHERE oi.order_id = $1`, 
            [req.params.id]
        );

        const history = await pool.query(
            'SELECT * FROM public.order_status_history WHERE order_id = $1 ORDER BY created_at DESC',
            [req.params.id]
        );
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        let updateQuery = 'UPDATE public.orders SET updated_at = CURRENT_TIMESTAMP';
        const queryParams = [];
        let paramCount = 1;
...
exports.getAllOrders = async (req, res) => {
    try {
        const orders = await pool.query('SELECT o.*, u.name as user_name FROM public.orders o LEFT JOIN public.users u ON o.user_id = u.id ORDER BY o.created_at DESC');
