...
exports.createBrand = async (req, res) => {
    try {
        ...
        const newBrand = await pool.query(
            'INSERT INTO public.brands (name, slug, logo, description, category_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
exports.updateBrand = async (req, res) => {
    try {
        ...
        const updatedBrand = await pool.query(
            'UPDATE public.brands SET name = COALESCE($1, name), slug = COALESCE($2, slug), category_id = $3, logo = COALESCE($4, logo), description = COALESCE($5, description) WHERE id = $6 RETURNING *',
...
exports.deleteBrand = async (req, res) => {
    try {
        const result = await pool.query('DELETE FROM public.brands WHERE id = $1', [req.params.id]);
